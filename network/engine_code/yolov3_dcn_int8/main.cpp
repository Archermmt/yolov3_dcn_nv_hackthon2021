...
int main(int argc, char** argv){
  ...
  if(!show_detail){
    ...
    repeat_num=argc>2? atoi(argv[2]):5;
  }
  ...
  if(!FileUtils::file_exist("yolov3_dcn_1.trt")){
    ...
    //set the calibrator
    DLRBatchStream calibrationStream("/usr/local/quake/datas/testset/yolov3_dcn",batch_size,-1,"yolov3_dcn_1.tensor_info");
    DLRInt8LegacyCalibrator calibrator(calibrationStream,0,"yolov3_dcn_1",1.0,0.9999);
    ...
    print_center("Engine of type int8 serialized to yolov3_dcn_1.trt");
  }
