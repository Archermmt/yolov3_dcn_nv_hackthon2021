...
bool infer(std::shared_ptr<ICudaEngine> engine,int batch_size,int repeat_num,bool show_detail,double& totalTime)
{
  ...
  const int output_5 = engine->getBindingIndex("ssh_m1_red_conv_relu");
  CHECK(cudaMalloc(&gpu_buffers[output_5],batch_size*720000*sizeof(float)));
  cpu_buffers[output_5]=malloc(batch_size*720000*sizeof(float));
  const int output_6 = engine->getBindingIndex("slice0");
  CHECK(cudaMalloc(&gpu_buffers[output_6],batch_size*720000*sizeof(float)));
  cpu_buffers[output_6]=malloc(batch_size*720000*sizeof(float));
  ...
  for(int i=0;i<repeat_num;i++){
    ...
    CHECK(cudaMemcpyAsync(cpu_buffers[output_5],gpu_buffers[output_5],batch_size*720000*sizeof(float),cudaMemcpyDeviceToHost,stream));
    CHECK(cudaMemcpyAsync(cpu_buffers[output_6],gpu_buffers[output_6],batch_size*720000*sizeof(float),cudaMemcpyDeviceToHost,stream));
    cudaStreamSynchronize(stream);
  ...
  failed_cnt=FileUtils::verify_buffer_with_file("ssh_m1_red_conv_relu","/usr/local/quake/datas/inference_datas/out_mismatch/ssh_m1_red_conv_relu.bin",(float*)cpu_buffers[output_5],batch_size*720000,0.05*100,show_detail);
  if(failed_cnt>batch_size*720000*0.05)
    passed=false;

  failed_cnt=FileUtils::verify_buffer_with_file("slice0","/usr/local/quake/datas/inference_datas/out_mismatch/slice0.bin",(float*)cpu_buffers[output_6],batch_size*720000,0.05*100,show_detail);
  if(failed_cnt>batch_size*720000*0.05)
    passed=false;
...
int main(int argc, char** argv){
  ...
  if(!FileUtils::file_exist("ssha_1.trt")){
    ...
    network->markOutput(*outputs[5]);
    network->markOutput(*outputs[6]);
